...
`;
		...
		function rolesForSurface(surface, accent, foreground) {
			const readableAccent = ensureContrast(accent, surface, 4.5);
			const readableForeground = ensureContrast(foreground, surface, 7);
			const secondary = ensureContrast(mixRgb(readableForeground, surface, .24), surface, 4.5);
			return {
		...
		function createThemeRoleModes(palette, accentIndex) {
			...
			return {
				light: rolesForSurface(mixRgb(sourceSurface, white, .9), pickedAccent, black),
				dark: rolesForSurface(mixRgb(sourceSurface, black, .62), pickedAccent, white)
			};
		}
		...
		function buildThemeTokens(palette, config) {
			...
			return {
				"--dsw-alias-bg-base": modes(rgba(lightSurface, .58 + strength * .24), rgba(darkSurface, .58 + strength * .24)),
				"--dsw-alias-bg-layer-1": modes(rgba(mixRgb(lightSurface, white, .04), .66 + strength * .22), rgba(mixRgb(darkSurface, white, .04), .66 + strength * .22)),
				"--dsw-alias-bg-layer-2": modes(rgba(mixRgb(lightSurface, white, .08), .7 + strength * .2), rgba(mixRgb(darkSurface, white, .08), .7 + strength * .2)),
				"--dsw-alias-bg-overlay": modes(rgba(lightSurface, .9 + strength * .08), rgba(darkSurface, .9 + strength * .08)),
				"--dsw-alias-border-l1": modes(rgba(lightForeground, .12), rgba(darkForeground, .12)),
				"--dsw-alias-border-l2": modes(rgba(lightForeground, .22), rgba(darkForeground, .22)),
				...
				"--dsw-specific-sidebar-fill": modes(rgba(lightSurface, .54 + strength * .24), rgba(darkSurface, .54 + strength * .24))
			};
		}
		//#endregion
